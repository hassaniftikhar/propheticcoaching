	...
	*/
	...
	if (var_check('application_settings_submit')) 
	{
		$result = $db->execute("
			...
			SET config_value = CASE 
				WHEN config_name = 'applications_guests' THEN '" . get_var('applications_guests') . "'
				WHEN config_name = 'hide_applications_menu' THEN '" . get_var('hide_applications_menu') . "'
	...
	if (var_check('chatroom_settings_submit')) 
	{
		$result = $db->execute("
			UPDATE arrowchat_config 
			SET config_value = CASE 
				WHEN config_name = 'user_chatrooms' THEN '" . get_var('user_chatrooms') . "'
				WHEN config_name = 'user_chatrooms_flood' THEN '" . get_var('user_chatrooms_flood') . "'
				WHEN config_name = 'user_chatrooms_length' THEN '" . get_var('user_chatrooms_length') . "'
				WHEN config_name = 'chatroom_auto_join' THEN '" . get_var('chatroom_auto_join') . "'
				WHEN config_name = 'chatroom_history_length' THEN '" . get_var('chatroom_history_length') . "'
			END WHERE config_name IN ('user_chatrooms', 'user_chatrooms_flood', 'user_chatrooms_length', 'chatroom_auto_join', 'chatroom_history_length')
		");
				...
				foreach ($usernames as $unbans) 
				{
					$db->execute("
						...
						WHERE user_id = '" . $db->escape_string($unbans) . "'
							AND chatroom_id = '" . get_var('chatroom_id') . "'
					");
				}
			} 
	...
	if (var_check('app_edit_submit')) 
	{
		...
		if (empty($error)) 
		{
			$result = $db->execute("
				UPDATE arrowchat_applications 
				SET name = '" . get_var('app_name') . "', 
					icon = '" . get_var('app_icon') . "', 
					folder = '" . get_var('app_folder') . "', 
					width = '" . get_var('app_width') . "', 
					height = '" . get_var('app_height') . "', 
					bar_width = '" . get_var('bar_width') . "', 
					bar_name = '" . get_var('bar_name') . "', 
					link = '" . get_var('app_url') . "', 
					dont_reload = '" . get_var('dont_reload') . "',
					show_to_guests = '" . get_var('show_to_guests') . "',
					default_bookmark = '" . get_var('default_bookmark') . "',
					update_link = '" . get_var('app_update_url') . "' 
				WHERE id = '" . get_var('app_id') . "'
			");
...
",  get_var('add_notification_markup'));
			...
			$result = $db->execute("
				...
				VALUES (
					'" . get_var('add_notification_name') . "', 
					'" . $db->escape_string($type) . "', 
					'" . stripslashes($markup_data) . "'
				)
			");
