<?php 
	...
	*/
	...
	function get_user_id() 
	{
		...
		{
			$result = $db->execute("
				SELECT session_auth_user_id 
				FROM " . TABLE_PREFIX . "session_auth 
				WHERE session_auth_key = '" . $db->escape_string($_COOKIE['se_auth_token']) . "'
			");

			if ($row = $db->fetch_array($result))
			{
				$userid = $row['session_auth_user_id'];
			}
		}
	...
	function get_friend_list($userid, $time) 
	{
		...
		$sql = ("
			SELECT DISTINCT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity, CONCAT(" . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " ,CONCAT('/'," . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . ")) avatar, " . TABLE_PREFIX . DB_USERTABLE . ".user_username link, arrowchat_status.is_admin, arrowchat_status.status 
	...
	function get_user_details($userid) 
	{
		...
		$sql = ("
			SELECT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity,  " . TABLE_PREFIX . DB_USERTABLE . ".user_username link, CONCAT(" . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " ,CONCAT('/'," . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . ")) avatar, arrowchat_status.is_admin, arrowchat_status.status 
			FROM " . TABLE_PREFIX . DB_USERTABLE . " 
			LEFT JOIN arrowchat_status 
				ON " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = arrowchat_status.userid 
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . "/uploads_user/1000/" . $image)) 
		{
			return $base_url . "../uploads_user/1000/" . $image;
		} 
		else 
		{
